...
func BenchmarkRequest_UnmarshalJSONReq(b *testing.B) {
	r := &Request{}

	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := r.UnmarshalJSONReq(data); err != nil {
			b.Fatal(err)
		}
	}
}

func BenchmarkRequest_UnmarshalJSONSimd(b *testing.B) {
	r := &Request{}

	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if err := r.UnmarshalJSONSimd(data); err != nil {
			b.Fatal(err)
		}
	}
}
